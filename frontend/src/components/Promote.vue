...
<v-card style="width:300px; margin-left:5%;" outlined>
    ...
    <v-card-title v-if="value._links">
        Promote # {{value._links.self.href.split("/")[value._links.self.href.split("/").length - 1]}}
    </v-card-title >
    <v-card-title v-else>
        Promote
    </v-card-title >
    ...
    <v-card-text style = "margin-left:-15px; margin-top:10px;">
          ...
          <div class="grey--text ml-4" v-if="editMode" style = "margin-top:-20px;">
            <v-text-field type="number" label="Phoneno" v-model="value.phoneno"/>
          </div>
          ...
          <div class="grey--text ml-4" v-if="editMode" style = "margin-top:-20px;">
            <v-text-field label="Couponkind" v-model="value.couponkind"/>
          </div>
          <div class="grey--text ml-4" v-else>
            Couponkind :  {{value.couponkind }}
          </div>

          <div class="grey--text ml-4" v-if="editMode" style = "margin-top:-20px;">
            <v-text-field label="Couponuseyn" v-model="value.couponuseyn"/>
          </div>
          <div class="grey--text ml-4" v-else>
            Couponuseyn :  {{value.couponuseyn }}
          </div>
    ...
    methods: {
      ...
      async save(){
        try{
          ...
          if(this.isNew){
            temp = await axios.post(axios.fixUrl('/promotes'), this.value)
          }else{
